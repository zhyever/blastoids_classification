...
from estimator.utils import colorize, get_model_param_dict
import tempfile
...
class Trainer:
    ...
    def log_images(self, log_dict, prefix="", scalar_cmap="turbo_r", min_depth=1e-3, max_depth=80, step=0):
        ...
        # add some thing for att log
        if 'addtional_vis' in log_dict.keys():
            log_img = log_dict.get('addtional_vis')[0][0]
            log_img = wandb.Image(
                log_img,
                caption='att_c')
            cur_log = wimages['{}/LogImageDepth'.format(prefix)]
            cur_log.append(log_img)
            
            log_img = log_dict.get('addtional_vis')[1][0]
            log_img = wandb.Image(
                log_img,
                caption='att_f')
            cur_log = wimages['{}/LogImageDepth'.format(prefix)]
        ...
        for idx, (batch_indices, batch_data) in enumerate(zip(loader_indices, self.val_dataloader)):
            ...
            if isinstance(result, list):
                for num_refine in range(len(result)):
                    metrics = dataset.get_metrics(batch_data_collect['depth_gt'], result[num_refine], disp_gt_edges=batch_data.get('boundary', None), additional_mask=log_dict.get('mask', None))
                    results_list[num_refine].extend([metrics])
    ...
    def train_epoch(self, epoch_idx):
        ...
        for idx, batch_data in pbar:
            ...
            if self.config.train_cfg.val_type == 'iter_base':
                if (self.train_step + 1) % self.config.train_cfg.val_interval == 0:
                    self.val_epoch()
                
    def save_checkpoint(self, epoch_idx):
        if self.config.get('lora', False):
            model_dict = get_model_param_dict(self.model.module)
        elif hasattr(self.model.module, 'get_save_dict'):
    ...
    def run(self):
        ...
        for epoch_idx in range(self.config.train_cfg.max_epochs):
            ...
            if (epoch_idx + 1) % self.config.train_cfg.val_interval == 0 and self.config.train_cfg.val_type == 'epoch_base':
                self.val_epoch()
